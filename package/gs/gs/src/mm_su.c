...
int main(int argc, const char * argv[])
{
    ...
    if (open_connect_modbus(ctx, device_to_open, MB_HIGH_BAUDRATE) == 0)
    {
        ...
        // 3) depending on MAP, read out connection speed. is it in match? REG_F_BAUDRATE for FLOMAC,
        // 4) if we are NOT in FloMAC - switch to Flomac
        // 5) write all recommended field before switching, switch speed
        // 6) reconnect with new speed
